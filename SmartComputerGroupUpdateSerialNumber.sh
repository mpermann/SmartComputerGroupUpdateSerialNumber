...
# Name: SmartComputerGroupUpdateSerialNumber.sh
# Date: 04-23-2022
...
# Purpose: Updates smart computer group using group name and list of serial numbers.
...
# Usage: SmartComputerGroupUpdateSerialNumber.sh "Smart_Computer_Group_Name" "/path/to/list/of/serial/numbers"
...
smartGroupNameStatusCode=$(/usr/bin/curl -o /dev/null -w "%{http_code}" -s -k -u "$APIUSER":"$APIPASS" "$JPSURL"/JSSResource/computergroups/name/"${smartGroupName// /%20}")
...
then
    ...
    smartGroupID=$(/usr/bin/curl -s -k -u "$APIUSER":"$APIPASS" "$JPSURL"/JSSResource/computergroups/id/0 -X POST -H Content-type:application/xml --data "<computer_group><name>$smartGroupName</name><is_smart>true</is_smart><site><id>-1</id><name>None</name></site></computer_group>" | xpath -e /computer_group/id | tr -cd "[:digit:]")
    /bin/echo "$smartGroupName created with ID of: $smartGroupID"
else
    /bin/echo "Group is valid"
    smartGroupID=$(/usr/bin/curl -s -k -u "$APIUSER":"$APIPASS" "$JPSURL"/JSSResource/computergroups/name/"${smartGroupName// /%20}" | xpath -e /computer_group/id | tr -cd "[:digit:]")
...
groupXML="<computer_group><criteria>"
...
groupXML="$groupXML</criteria></computer_group>"
...
/bin/echo "Attempting to upload computers to group $smartGroupID"
/usr/bin/curl -s -k -u "$APIUSER":"$APIPASS" "$JPSURL"/JSSResource/computergroups/id/"$smartGroupID" -X PUT -H Content-type:application/xml --data "$groupXML"